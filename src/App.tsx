...
const CONFIG = {
    CONTRACT_ADDRESS: "0xfaD03306adaa4e465CEf4055a9B1385342FD702f",
...
const ABI = [
    "function recordIdea(string _originalHash, string _ipfsCID) public",
    "function getMyRecordsCount() public view returns (uint256)",
    "function getRecord(uint256 _index) public view returns (string memory, string memory, uint256, uint256)"
];
...
function App() {
    ...
    const checkTokenBalance = async (walletAddress: string) => {
        try {
            ...
            const balanceInUnits = ethers.utils.formatUnits(balance, 18);
    ...
    const checkTokenAllowance = async (walletAddress: string) => {
        try {
            const ethereum = (window as unknown as Record<string, unknown>).ethereum;
            const provider = new ethers.providers.Web3Provider(ethereum as ethers.providers.ExternalProvider);
            const tokenContract = new ethers.Contract(CONFIG.TOKEN_ADDRESS, TOKEN_ABI, provider);
            const allowance = await tokenContract.allowance(walletAddress, CONFIG.CONTRACT_ADDRESS);
            const allowanceInUnits = ethers.utils.formatUnits(allowance, 18);
            setTokenAllowance(allowanceInUnits);
            // console.log('✅ Token allowance:', allowanceInUnits, 'VP');
            return allowance;
        } catch (e) {
    ...
    const approveTokens = async () => {
        ...
        try {
            ...
            const tokenContract = new ethers.Contract(CONFIG.TOKEN_ADDRESS, TOKEN_ABI, signer);
            
            const approvalAmount = ethers.utils.parseUnits(CONFIG.NOTARY_FEE, 18);
            const tx = await tokenContract.approve(CONFIG.CONTRACT_ADDRESS, approvalAmount);
            
            // console.log('⏳ Approval pending:', tx.hash);
            setStatus('⏳ Processing approval on blockchain...');
            ...
            await checkTokenAllowance(userAddress);
            ...
            const errorMessage = typeof e === 'object' && e !== null
                ? (errorObj.message as string) || (errorObj.reason as string) || JSON.stringify(e)
                : String(e);
            ...
            if (errorMessage?.includes('user rejected')) {
                errorMsg = '⚠️ You rejected the token approval';
            } else {
    ...
    const loadLedger = async (forcedUser?: string) => {
        ...
        try {
            ...
            const contract = new ethers.Contract(CONFIG.CONTRACT_ADDRESS, ABI, provider.getSigner());
            ...
            for (let i = count - 1; i >= Math.max(0, count - 15); i--) {
                ...
                results.push({
                    index: i,
                    hash: r[0], stealthCID: r[1],
                    date: new Date(r[2].toNumber() * 1000).toLocaleString(),
                    version: r[3].toNumber()
                });
            }
            setRecords(results);
        } catch (e) {
    ...
    const anchorToBlockchain = async () => {
        ...
        try {
            ...
            const requiredFee = ethers.utils.parseUnits(CONFIG.NOTARY_FEE, 18);
            if (balance.lt(requiredFee)) {
                const balanceFormatted = ethers.utils.formatUnits(balance, 18);
                setStatus(`❌ Insufficient VP tokens (have ${balanceFormatted}, need ${CONFIG.NOTARY_FEE})`);
                return;
            }
            ...
            const allowance = await checkTokenAllowance(userAddress);
            if (!allowance) {
                setStatus('❌ Failed to check token approval');
                return;
            }
            
            // Step 3: If allowance is insufficient, show approval modal
            if (allowance.lt(requiredFee)) {
                //console.log('📝 Token approval needed');
                setShowApprovalModal(true);
                setStatus('⚠️ Token approval required');
                return;
            }
            
            //console.log('✅ Token balance and allowance verified');
            ...
            try {
               ...
               // console.log('  - Gas Limit:', gasEstimateNum.toLocaleString(), 'units');
                ...
                //console.log('🟢 Modal displayed with real gas data');
            } catch (_gasError) {
                void _gasError;
                //console.log('⚠️ Gas estimation error:', _gasError);
                //console.log('⚠️ Will estimate gas during transaction');
    ...
    const executeTxSubmission = async () => {
        //console.log('🟠 executeTxSubmission START');
        
        if (!sessionData.stealthCID) {
            //console.log('❌ No stealthCID');
            setStatus('⚠️ No data to anchor');
        ...
        try {
            //console.log('🟡 Get ethereum from window');
            const ethereum = (window as unknown as Record<string, unknown>).ethereum;
            if (!ethereum) throw new Error('MetaMask not found');
            ...
            //console.log('🟡 Ready to call contract.recordIdea()');
            ...
            //console.log('🟢 TX RETURNED:', tx.hash);
            ...
            //console.log('🟢 Receipt:', receipt.blockNumber, receipt.gasUsed.toString());
    ...
    return (
        <div className="relative min-h-screen w-full flex flex-col items-center justify-center p-6 bg-linear-to-b from-slate-950 to-slate-900" role="main">
            {status && (
                <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 px-6 py-3 bg-indigo-600/90 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-2xl shadow-indigo-600/40 animate-pulse transition-all duration-300" role="alert">
                    {status}
                </div>
            )}

            {copiedNotification && (
                <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 px-4 py-2 bg-emerald-600/90 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-2xl shadow-emerald-600/40 animate-pulse transition-all duration-300" role="alert">
                    Copied!
                </div>
            )}
            ...
            {showTxDetails && txDetails && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex items-center justify-center p-4">
                    <div className="bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 border border-indigo-500/30 rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-6">
                        ...
                        <div className="space-y-3 pt-4 border-t border-slate-700">
                            <div className="flex justify-between items-start">
                                ...
                                <span className="text-white font-semibold">{CONFIG.HEDERA_NETWORK_NAME}</span>
                            </div>
                            <div className="flex justify-between items-start">
                                <span className="text-slate-400 text-sm">Contract</span>
                                <span className="text-white font-mono text-xs truncate max-w-xs">{CONFIG.CONTRACT_ADDRESS.slice(0, 10)}...{CONFIG.CONTRACT_ADDRESS.slice(-8)}</span>
                            </div>
                        </div>

                        {/* File Details */}
                        <div className="space-y-3 pt-4 border-t border-slate-700">
                            <h4 className="text-slate-300 font-semibold text-sm">File Details</h4>
                            <div className="space-y-2 pl-3 border-l-2 border-indigo-500/50">
                                <div>
                                    <p className="text-slate-400 text-xs">File Name</p>
                                    <p className="text-white font-mono text-sm truncate">{txDetails.fileName}</p>
                                </div>
                                <div>
                                    <p className="text-slate-400 text-xs">Original Hash</p>
                                    <p className="text-white font-mono text-xs break-all">{txDetails.originalHash}</p>
                                </div>
                                <div>
                                    <p className="text-slate-400 text-xs">Stealth CID</p>
                                    <p className="text-white font-mono text-xs break-all">{txDetails.stealthCID}</p>
                                </div>
                            </div>
                        </div>

                        {/* Gas & Cost Details */}
                        <div className="space-y-3 pt-4 border-t border-slate-700 bg-slate-800/50 rounded-lg p-4">
                            <h4 className="text-slate-300 font-semibold text-sm">💰 Transaction Cost Breakdown</h4>
                            
                            {/* Gas Limit */}
                            <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-400 text-xs font-medium">Gas Limit</span>
                                    <span className="text-indigo-300 font-mono font-bold">{txDetails.estimatedGas !== '0' ? parseInt(txDetails.estimatedGas).toLocaleString() : 'Calculating...'} units</span>
                                </div>
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-400 text-xs font-medium">Gas Price</span>
                                    <span className="text-cyan-300 font-mono font-bold">{parseFloat(txDetails.gasPrice).toFixed(3)} gwei</span>
                                </div>
                                <div className="h-0.5 bg-slate-700 rounded"></div>
                            </div>
                            
                            {/* Total Cost */}
                            <div className="pt-2 flex justify-between items-center bg-indigo-600/10 rounded p-3 border border-indigo-500/20">
                                <span className="text-slate-200 font-semibold text-sm">Total Fee</span>
                                <div className="text-right">
                                    <span className="text-indigo-300 font-mono font-bold text-lg block">{parseFloat(txDetails.estimatedCost).toFixed(6)} HBAR</span>
                                    <span className="text-slate-400 text-xs">(Hedera Testnet)</span>
                                </div>
                            </div>
                            
                            <p className="text-slate-400 text-xs pt-2 italic">
                        ...
                        <div className="flex gap-3 pt-4">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    //console.log('isAnchoring:', isAnchoring);
                                    //console.log('sessionData:', sessionData);
                                    //console.log('About to call executeTxSubmission...');
                                    await executeTxSubmission();
                                    //console.log('executeTxSubmission completed');
                                }}
                                disabled={isAnchoring}
                                className="flex-1 px-4 py-2 rounded-lg bg-linear-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 disabled:opacity-50 text-white font-semibold transition-all"
                            >
            ...
            {showApprovalModal && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex items-center justify-center p-4">
                    <div className="bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 border border-yellow-500/30 rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-6">
                        <div className="flex items-center justify-between">
                            <h3 className="text-xl font-bold text-white">Token Approval Required</h3>
                            <button
                                onClick={() => {
                                    setShowApprovalModal(false);
                                    setStatus('⚠️ Token approval cancelled');
                                }}
                                className="text-slate-400 hover:text-white text-2xl leading-none"
                            >
                                ✕
                        ...
                        <div className="space-y-3 pt-4 border-t border-slate-700">
                            <p className="text-slate-300 text-sm">
                                The VaultNotary contract needs approval to spend your VP tokens.
                            </p>
                        </div>

                        {/* Token Details */}
                        <div className="space-y-3 pt-4 border-t border-slate-700 bg-slate-800/50 rounded-lg p-4">
                            <h4 className="text-slate-300 font-semibold text-sm">💰 Token Details</h4>
                            
                            {/* Your Balance */}
                            <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-400 text-xs font-medium">Your VP Balance</span>
                                    <span className="text-emerald-300 font-mono font-bold">{parseFloat(tokenBalance).toFixed(2)} VP</span>
                                </div>
                                <div className="h-0.5 bg-slate-700 rounded"></div>
                            </div>
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-400 text-xs font-medium">Required Amount</span>
                                    <span className="text-yellow-300 font-mono font-bold">{CONFIG.NOTARY_FEE} VP</span>
                                </div>
                                <div className="h-0.5 bg-slate-700 rounded"></div>
                            </div>
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-400 text-xs font-medium">Current Allowance</span>
                                    <span className="text-slate-400 font-mono font-bold">{parseFloat(tokenAllowance).toFixed(2)} VP</span>
                                </div>
                                <div className="h-0.5 bg-slate-700 rounded"></div>
                            </div>
                            
                            <p className="text-slate-400 text-xs pt-2 italic">
                                This approval allows the contract to charge {CONFIG.NOTARY_FEE} VP tokens per transaction.
                            </p>
                        </div>
                        ...
                        <div className="flex gap-3 pt-4">
                            ...
                            <button
                                ...
                                className="flex-1 px-4 py-2 rounded-lg bg-linear-to-r from-yellow-600 to-yellow-500 hover:from-yellow-500 hover:to-yellow-400 disabled:opacity-50 text-white font-semibold transition-all"
                            >
                        ...
                        <p className="text-slate-400 text-xs text-center">
                            MetaMask will open for you to confirm the token approval.
                        </p>
                    </div>
                </div>
            )}
            ...
            <nav className="absolute top-0 w-full p-8 flex justify-between items-center max-w-7xl">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-lg font-black text-white">VaultProtocol</h1>
                        <p className="text-[10px] text-slate-500">Stealth Notary</p>
                    </div>
                </div>
                
                <div className="flex gap-2">
                    <button onClick={() => setView('anchor')} className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition ${view === 'anchor' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-white'}`}>
                        Anchor
                    </button>
                    <button onClick={() => setView('verify')} className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition ${view === 'verify' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-white'}`}>
                        Verify
                    </button>
                    <button onClick={() => setView('ledger')} className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition ${view === 'ledger' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-white'}`}>
                        Ledger
                    </button>
                    <button onClick={connectWallet} className="ml-4 border border-slate-700 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-slate-800 transition text-slate-300">
            ...
            {view === 'anchor' && !showReceipt && (
                <div className="w-full max-w-md">
                    <div className="flex justify-center gap-2 mb-10">
                        {[1, 2, 3].map(step => (
                            <div key={step} className={`h-2 w-32 ${anchorStep >= step ? 'bg-indigo-600' : 'bg-slate-800'} rounded-full transition`} />
                        ))}
                    </div>

                    <div className="bg-slate-900/40 backdrop-blur-md border border-slate-800 rounded-[2.5rem] p-12 text-center">
                        {anchorStep === 1 && (
                            <div className="space-y-6">
                                <Icon name="file" size={60} className="mx-auto text-indigo-400" />
                                <h2 className="text-2xl font-bold text-white">Step 1: Seal Asset</h2>
                                 ...
                                 <label 
                                     htmlFor="file-upload"
                                     className="w-full bg-slate-950/50 border-2 border-dashed border-slate-700 rounded-2xl p-8 text-sm text-slate-400 block cursor-pointer hover:border-indigo-500/50 hover:text-indigo-400 transition-all focus-within:ring-2 focus-within:ring-indigo-500/50"
                                 >
                                 ...
                                 <input 
                                     type="password"
                                     className="w-full bg-slate-950/50 border border-slate-800 rounded-2xl p-4 text-sm text-white outline-none focus:border-indigo-500/50 transition-all focus:ring-2 focus:ring-indigo-500/50"
                                 ...
                                 <p className="text-slate-300 text-xs bg-slate-800/50 p-3 rounded-lg border border-slate-700">💡 <strong>Tip:</strong> You can skip the password and use <strong>'Chain Hash Only'</strong> to anchor without encryption</p>
                                
                                 <button 
                                     onClick={encryptLocally}
                                     disabled={isEncrypting}
                                     className="w-full bg-indigo-600 py-4 rounded-3xl text-[11px] font-black uppercase tracking-widest text-white hover:bg-indigo-500 disabled:opacity-50 transition-all flex items-center justify-center"
                                 >
                                     {isEncrypting ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Encrypting...</>) : "Encrypt and Back Up"}
                                 </button>
                                
                                 <button 
                                     onClick={anchorHashOnly}
                                     disabled={isEncrypting}
                                     className="w-full bg-slate-800 hover:bg-slate-700 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-300 transition-all border border-slate-700 flex items-center justify-center"
                                 >
                                     {isEncrypting ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Hashing...</>) : (<><Icon name="bolt" className="mr-2" size={16} />Chain Hash Only</>)}
                                 </button>
                            </div>
                        )}
                        ...
                        {anchorStep === 2 && (
                            <div className="space-y-6">
                                <Icon name="cloud" size={48} className="mx-auto text-indigo-400" />
                                <h2 className="text-2xl font-bold text-white">Step 2: Back Up</h2>
                                <p className="text-slate-400 text-sm">Upload to cloud or anchor hash only</p>
                                 ...
                                 <button 
                                     ...
                                     disabled={isMasking}
                                     className="w-full bg-indigo-600 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-white hover:bg-indigo-500 disabled:opacity-50 transition-all flex items-center justify-center"
                                 >
                                     {isMasking ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Uploading...</>) : "Upload to IPFS"}
                                 </button>
                                     ...
                                     <div className="mt-4 p-4 bg-slate-800/50 border border-amber-500/30 rounded-xl space-y-4 animate-in slide-in-from-top-2 duration-300">
                                         <div className="flex items-start gap-3">
                                             ...
                                             <div className="flex-1">
                                                 ...
                                                 <input
                                                     ...
                                                     autoComplete="off"
                                                     spellCheck="false"
                                                     className="w-full bg-slate-900/50 border border-slate-600 rounded-lg px-3 py-2 text-white placeholder-slate-400 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 font-mono"
                                                     autoFocus
                                                 />
                                                 {jwtError && (
                                         ...
                                         <div className="flex gap-2">
                                             <button
                                                 ...
                                                 className="flex-1 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-semibold py-2 px-3 rounded-lg transition-all"
                                             >
                                                 Upload to IPFS
                                             </button>
                                             <button
                                                 ...
                                                 className="px-3 py-2 bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-semibold rounded-lg transition-all"
                                             >
                                                 Cancel
                                             </button>
                                         </div>
                                 ...
                                 <button 
                                     ...
                                     className="w-full bg-slate-800 hover:bg-slate-700 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-300 transition-all border border-slate-700 flex items-center justify-center"
                                 >
                                     <Icon name="bolt" className="mr-2" size={16} /> Skip - Chain Hash Only
                                 </button>
                            </div>
                        )}
                        ...
                        {anchorStep === 3 && (
                            <div className="space-y-6">
                                <Icon name="lock" size={60} className="mx-auto text-indigo-400" />
                                <h2 className="text-2xl font-bold text-white">Step 3: Anchor Proof</h2>
                                <p className="text-slate-400 text-sm">
                                 ...
                                 <button 
                                     ...
                                     className="w-full bg-indigo-600 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-white hover:bg-indigo-500 disabled:opacity-50 transition-all flex items-center justify-center"
                                 >
                                     {isAnchoring ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Anchoring...</>) : "Anchor to Blockchain"}
                                 </button>
                            </div>
                        )}
                    </div>
            ...
            {showReceipt && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="w-full max-w-xl bg-slate-900/40 backdrop-blur-md border border-slate-800 rounded-[2.5rem] p-12 text-center">
                        <h2 className="text-3xl font-bold text-white mb-2">Stealth Anchor Complete</h2>
                        <p className="text-slate-500 text-xs font-medium mb-10 uppercase tracking-widest">Confirmed on Hedera Testnet</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-10">
                            <div className="p-4 bg-slate-950/50 border border-slate-800 rounded-lg">
                                <p className="text-[10px] text-slate-500 mb-1">Block</p>
                                <p className="text-lg font-mono text-indigo-400">{sessionData.blockNum}</p>
                            </div>
                            <div className="p-4 bg-slate-950/50 border border-slate-800 rounded-lg">
                                <p className="text-[10px] text-slate-500 mb-1">Time</p>
                                <p className="text-xs font-mono text-indigo-400">{sessionData.chainTime}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-3 mt-8">
                            <button 
                                ...
                                className="w-full bg-emerald-600 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest text-white hover:bg-emerald-500 transition flex items-center justify-center"
                            >
                                <Icon name="download" className="mr-2" size={16} /> Download Audit
                            </button>
                            <button 
                                onClick={resetAnchor}
                                className="w-full bg-indigo-600 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest text-white hover:bg-indigo-500 transition"
                            >
                                Close
                            </button>
                        </div>
            ...
            {view === 'verify' && (
                <div className="w-full max-w-lg bg-slate-900/40 backdrop-blur-md border border-slate-800 rounded-[2.5rem] p-10">
                    <div className="flex gap-2 mb-8 bg-slate-900/50 p-1.5 rounded-2xl border border-slate-800/50">
                        <button 
                            onClick={() => setVerifyTab('decrypt')}
                            className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase transition ${verifyTab === 'decrypt' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}
                        >
                            Decrypt
                        </button>
                        <button 
                            onClick={() => setVerifyTab('unmask')}
                            className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase transition ${verifyTab === 'unmask' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}
                        >
                            Unmask
                        </button>
                    </div>
                    ...
                    {verifyTab === 'decrypt' && (
                        <div className="space-y-4">
                                 ...
                                 <label 
                                     ...
                                     className="w-full bg-slate-950/50 border-2 border-dashed border-slate-700 rounded-lg p-6 text-sm text-slate-400 block cursor-pointer hover:border-indigo-500/50 focus-within:ring-2 focus-within:ring-indigo-500/50"
                                 >
                                     {fileToDecrypt ? `📄 ${fileToDecrypt.name}` : "Select encrypted file"}
                                 </label>
                                 <input 
                                     type="password"
                                     className="w-full bg-slate-950/50 border border-slate-800 rounded-lg p-3 text-sm text-white outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/50"
                                 ...
                                 <button 
                                     ...
                                     className="w-full bg-indigo-600 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest text-white hover:bg-indigo-500 disabled:opacity-50 flex items-center justify-center"
                                 >
                                     {isVerifying ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Decrypting...</>) : "Decrypt File"}
                                 </button>
                            {decryptedResult && (
                                <div className="mt-4 p-4 bg-slate-950/60 border border-slate-800 rounded-lg">
                                    <p className="text-[10px] uppercase text-slate-500 mb-2">Decrypted asset</p>
                                    <p className="font-mono text-[11px] text-indigo-300 truncate">{decryptedResult.name}</p>
                                    <p className="text-[10px] text-slate-400 mt-2">Hash: {decryptedResult.hash}</p>
                                    <a 
                                        href={URL.createObjectURL(decryptedResult.blob)}
                                        download={decryptedResult.name}
                                        className="mt-3 inline-flex items-center justify-center w-full bg-emerald-600 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest text-white hover:bg-emerald-500"
                                    >
                    ...
                    {verifyTab === 'unmask' && (
                        <div className="space-y-4">
                                 ...
                                 <button 
                                     ...
                                     className="w-full bg-indigo-600 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest text-white hover:bg-indigo-500 disabled:opacity-50 flex items-center justify-center"
                                 >
                                     {isVerifying ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Unmasking...</>) : "Unmask CID"}
                                 </button>
                            {unmaskedCID && (
                                <div className="space-y-3">
                                    ...
                                    <button 
                                        ...
                                        disabled={isVerifying}
                                        className="w-full bg-emerald-600 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest text-white hover:bg-emerald-500 disabled:opacity-50 flex items-center justify-center"
                                    >
                                        {isVerifying ? (<><Icon name="loader2" className="animate-spin mr-2" size={16} /> Downloading...</>) : (<><Icon name="download" className="mr-2" size={16} /> Download from IPFS</>)}
                                    </button>
                                </div>
                            )}
                        </div>
            ...
            {view === 'ledger' && (
                <div className="w-full max-w-3xl">
                    <h2 className="text-2xl font-bold text-white mb-8 text-center">System Ledger</h2>
                    <div className="space-y-4">
                        {records.length === 0 ? (
                            ...
                            records.map((record, i) => (
                                <div key={i} className="bg-slate-900/40 border border-slate-800 rounded-lg p-4">
                                    <p className="text-[10px] text-slate-500 uppercase mb-2">Record #{record.index}</p>
                                    ...
                                    <p className="text-[10px] text-slate-400 mt-2">{record.date}</p>
                                </div>
                            ))
                        )}
                    </div>
            ...
            <div className="fixed bottom-4 w-full text-center pointer-events-none">
                <p className="text-[9px] text-slate-600 uppercase tracking-[0.3em] font-bold">
                    VaultProtocol Secure Node • AES-256 + ZK-Masking 2026
                </p>
            </div>
        </div>
    );
